...
public class MenuFragment2 extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        v.findViewById(R.id.btnUsers2).setOnClickListener(view -> {
        ...
        v.findViewById(R.id.btnInvoices2).setOnClickListener(view -> {
            open(new InvoicesFragment());
        });

        v.findViewById(R.id.btnThongtin2).setOnClickListener(view -> {
            open(new ProfileFragment()); // TODO: tạo TopProductsFragment (có thể là list top)
        });

        v.findViewById(R.id.btnChangePass2).setOnClickListener(view -> {
            showChangePasswordDialog();
        });

        v.findViewById(R.id.btnLogout2).setOnClickListener(view -> {
            confirmLogout();
        });
    ...
    private void open(Fragment f) {
        requireActivity().getSupportFragmentManager()
                .beginTransaction()
                .replace(R.id.flContent3, f) // đổi thành id container thực tế của bạn
                .addToBackStack(null)
                .commit();
    }
